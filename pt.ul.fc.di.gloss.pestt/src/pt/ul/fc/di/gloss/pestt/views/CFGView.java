package pt.ul.fc.di.gloss.pestt.views;
...
import pt.ul.fc.di.gloss.pestt.domain.NodeContentProvider;
...
public class CFGView extends ViewPart implements IZoomableWorkbenchPart {
	 ...
	 */
	public static final String ID = "exemplo.views.SampleView";
	...
	*/
	...
	public void createPartControl(Composite parent) {
		...
		viewer.setContentProvider(new NodeContentProvider(model));
		viewer.setLabelProvider(new LabelContentProvider(model));
		//viewer.setSorter(new NameSorter());
		//viewer.setInput(getViewSite());
		viewer.setInput(model.getNodes());
		
		// Law the nodes of the graph
		...
		PlatformUI.getWorkbench().getHelpSystem().setHelp(viewer.getControl(), "exemplo.viewer");
		makeActions();
		hookContextMenu();
	...
	private void hookContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			public void menuAboutToShow(IMenuManager manager) {
				CFGView.this.fillContextMenu(manager);
			}
		});
